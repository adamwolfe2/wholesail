...
const DEFAULT_STATS = [
  { stat: "68%", label: "of B2B distributors still take orders by phone or email", source: "Conexiom Distribution Survey" },
  { stat: "3–4 hrs", label: "per day the average distribution team spends on order entry and follow-up", source: "Connexion, 2024" },
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <section className="py-20 sm:py-28" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          >
            {config.h1Line1}
            <br />
            <span style={{ color: "var(--blue)" }}>{config.h1Line2}</span>
          </h1>
          <p
        ...
        <section className="py-16" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          >
            {config.features.map((f, i) => (
              <div
                key={i}
                className={`p-5 flex items-start gap-3 ${i < config.features.length - 1 ? "border-b sm:border-b-0 sm:border-r" : ""}`}
                style={{ borderColor: "var(--border-strong)", backgroundColor: "var(--bg-white)" }}
              >
        ...
        <section className="py-16" style={{ borderBottom: "1px solid var(--border)" }}>
          {config.testimonial && (
            <div
              className="p-4 sm:p-8 lg:p-12 mb-8"
