...
import { products as staticProducts, productCategories as staticCategories } from '@/lib/products'
...
export default function CatalogPage() {
  ...
  useEffect(() => {
    fetch('/api/inventory').then(r => r.json()).then(setLowStock).catch(() => {})
  }, [])
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-[#E5E1DB]">
                  {filteredProducts.map(product => (
                    <div key={product.id} className="bg-[#F9F7F4]">
                      <ProductCard product={product} isSignedIn={!!isSignedIn} quantityOnHand={lowStock[product.id] ?? null} />
                    </div>
                  ))}
                </div>
              )}
