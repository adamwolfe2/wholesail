...
export default function PickListError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      console.error('[admin/fulfillment/pick-list]', error.digest ?? error.message)
    }
  }, [error])
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <div className="flex gap-3">
        ...
        <Button variant="outline" onClick={() => window.location.href = '/admin/fulfillment'}>
          Back to fulfillment
        </Button>
      </div>
      {error.digest && (
