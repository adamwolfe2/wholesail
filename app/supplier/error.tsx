...
export default function SupplierError({
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      console.error('[supplier]', error.digest ?? error.message)
    }
  }, [error])

  return (
    <div className="flex flex-col items-center justify-center py-24 text-center bg-cream text-ink px-4">
      ...
      <p className="font-mono text-sm text-ink/60 mb-8 max-w-sm leading-relaxed">
        This page failed to load. Try refreshing — if the issue persists, contact support.
      </p>
