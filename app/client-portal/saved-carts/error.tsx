...
export default function SavedCartsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      console.error('[client-portal/saved-carts]', error.digest ?? error.message)
    }
  }, [error])
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <div className="flex gap-3">
        ...
        <Button variant="outline" onClick={() => window.location.href = '/client-portal/saved-carts'}>
          Back to saved carts
        </Button>
      </div>
      {error.digest && (
