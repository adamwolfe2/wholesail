...
export default function PortalQuoteDetailError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      console.error('[client-portal/quotes/[id]]', error.digest ?? error.message)
    }
  }, [error])
    ...
    <PortalLayout>
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" onClick={() => window.location.href = '/client-portal/quotes'}>
            Back to quotes
          </Button>
        </div>
        {error.digest && (
