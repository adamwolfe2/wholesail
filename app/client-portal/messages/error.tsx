...
export default function PortalMessagesError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'production') {
      console.error('[client-portal/messages]', error.digest ?? error.message)
    }
  }, [error])
    ...
    <PortalLayout>
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" onClick={() => window.location.href = '/client-portal'}>
            Back to dashboard
          </Button>
        </div>
        {error.digest && (
