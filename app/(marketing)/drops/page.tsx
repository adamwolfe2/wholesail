...
function QuantityBar({ total, sold }: { total: number; sold: number }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-1 w-full bg-shell overflow-hidden">
        <div
...
function DropCard({ drop }: { drop: Drop }) {
  ...
  return (
    <div className="bg-cream p-6 sm:p-8 flex flex-col gap-4 min-h-[260px]">
      {/* Category + Featured */}
      <div className="flex items-center justify-between gap-2">
        ...
        {drop.featured && (
          <span className="text-[10px] tracking-[0.15em] uppercase bg-ink text-cream px-2 py-0.5">
            Featured
          </span>
        )}
      </div>
      ...
      <div className="flex-1">
        <h3 className="font-serif text-xl sm:text-2xl font-bold text-ink mb-2 leading-tight">
          {drop.title}
        </h3>
        {drop.description && (
      ...
      {/* Date + CTA */}
      <div className="pt-3 border-t border-shell space-y-3">
        ...
        {isAvailable ? (
          <Link
            ...
            className="inline-flex items-center gap-2 border border-ink text-ink px-5 py-2.5 text-sm font-medium hover:bg-ink hover:text-cream transition-colors"
          >
            Add to Cart
          </Link>
        ) : (
          <div className="space-y-3">
            <p className="text-[11px] uppercase tracking-wider text-sand font-medium">
      ...
      {(available.length > 0 || drops.length === 0) && (
        <section className="py-16 sm:py-20 border-b border-shell">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {available.length === 0 ? (
              <div className="border border-shell bg-white p-12 text-center">
